"""Сборка полей Lead из карточки лидогена."""
...
# При dedup по ИНН — обновляем эти поля; stage не трогаем.
LEADGEN_MERGE_FIELDS = (
    "company",
    "contact",
    "email",
    "phone",
...
def lead_payload_from_card(card: dict[str, Any]) -> dict[str, Any]:
    """Поля ORM Lead из карточки analyze (без stage — задаётся при insert)."""
    lpr = card.get("lpr", {}) or {}
    ...
    if fin.get("revenue"):
        notes_parts.append(
            f"Выручка: {_fmt_money(fin['revenue'])} ({fin.get('finance_year', '—')} г.)"
        )
    if card.get("hook"):
    ...
    return {
        ...
        "description": "\n\n".join(notes_parts),
        "inn": (card.get("inn") or "").strip() or None,
        "ogrn": card.get("ogrn", ""),
...
def apply_payload_to_lead(lead: Any, payload: dict[str, Any]) -> None:
    for key in LEADGEN_MERGE_FIELDS:
