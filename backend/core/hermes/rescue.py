...
def rescue_route(text: str) -> dict | None:
    ...
    if ("покажи" in t or "pokazi" in t or "список" in t) and (
        "лид" in t or "сделк" in t or "lidy" in t or "lid" in t
    ):
        return out("list_leads", ["analyst"], "Показываю лиды.", {"filter": "all"})

    if ("удали" in t or "удалить" in t or "delete" in t) and ("лид" in t or "компан" in t):
        return out("delete_lead", ["analyst"], "Удаляю лид.")
    ...
    if any(s in t for s in create_strong_signals) and not is_update_cue:
        return out("create_lead", ["analyst"], "Создаю лид.")
    if any(s in t for s in create_weak_signals) and ("создай" in t or "добавь" in t) and not is_update_cue:
        return out("create_lead", ["analyst"], "Создаю лид.")
    if "контакт" in t and not is_update_cue:
        return out("create_lead", ["analyst"], "Создаю лид.")
    ...
    if is_update_cue:
        return out("update_lead", ["analyst"], "Обновляю лид.")
...
def post_verify(text: str, parsed: dict) -> dict:
    if not config.HERMES_ENABLE_POST_VERIFY or not parsed:
        return parsed
    t = normalize_text_for_cache(text)
    out = dict(parsed)
