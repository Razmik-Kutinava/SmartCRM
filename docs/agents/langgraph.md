...
**Тендерный поиск** вынесен в отдельный модуль API/UI (**`/api/tenders`**, вкладка «Тендеры»), это не отдельный узел LangGraph. Описание источников, лимитов и env: [../modules/tenders.md](../modules/tenders.md).
...
- `update_lead` — загружает полный лид из БД по company, подтягивает задачи; **запись `score` в БД** через `update_lead_score` зависит от настроек CRM (см. ниже)
...
**Скоринг:** двойной — эвристика (`compute_lead_score`) + LLM, финальный = среднее двух **в ответе модели**. Запись в поле CRM `score`: по умолчанию **выключена** — `crm_settings.json` → `manager_sets_score: true`, `agents_may_update_score: false`. Тогда `update_lead_score` возвращает `skipped: true` и **не** вызывает `PATCH /api/leads/{id}` по `score`. Подсказка и предупреждения для людей — в `GET /api/leads/{id}` как `scoreAdvisory` (см. `docs/product/PRD.md`, `core/lead_score_advisory.py`). Включить запись скора агентом можно в Ops (`agents_may_update_score: true`).
...
**Фаза 2 — выборочный fanout:** Стратег подбирает подмножество агентов по задаче (например «анализ + письмо» → Аналитик ∥ Маркетолог; «тендер» → Тех.спец ∥ Экономист). Не тянуть Экономиста/Checko в каждый сценарий.

### Пакет `voice_action` (Стратег → UI)

После fanout Стратег формирует один JSON для WebSocket (см. `docs/product/ARCHITECTURE.md`):

- `ui: modal` — анализ + черновик письма + `actions: [send, save, reject]`
- `ui: navigate` — `route: /leads/{id}`
- `agents_used[]` — для трейса Ops (Фаза 3)
...
`ui_action`: `navigate` | `modal` | `filter` | `approve` — когда нужна реакция UI, не только API (`docs/product/PRD.md`).
