...
|-----------|------------|
| `all` | общая (все агенты) |
| `analyst` | аналитик |
| `economist` | экономист (книги, справочники) |
| `marketer` | маркетолог (B2B материалы) |
| `tec` | тех. спец (PDF, спецификации) |

Чанкинг: смысловой (абзацы + предложения). Форматы: PDF (нужен pypdf), DOCX, XLSX, CSV, JSON, TXT, MD.
...
| DataNewton | по ключу API | Данные по компаниям (обогащение лидов) |
