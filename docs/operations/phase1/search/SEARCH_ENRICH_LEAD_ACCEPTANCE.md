...
|----------|---------|-------------------|
| **Brave 429** при пиковой нагрузке | редко, backoff 45с | кэш Brave ✅; **кэш всего enrich → PRD_MAP Ф2 §4** |
| **revenue / linkedin** без ИНН | всё ещё слабо | Checko только с ИНН; мелкие компании — ручной дозапрос |
| Обогащение **с карточки лида** one-click | только `/search` | **PRD_MAP Ф2 §9** — кнопка на `/leads/{id}` |
| Зависимость **GROQ_API_KEY** | без LLM — только raw_results | как есть; общий ключ с Hermes |
| ~**60–90 с** на запрос | UX | **PRD_MAP Ф2 §4** — кэш enrich (company+ИНН, ~24 ч) |

---

## ❌ НЕ СДАЛИ → PRD_MAP Фаза 2

| Что | PRD_MAP |
|-----|---------|
| Кэш **всего** enrich-ответа (не только Brave) | **§4** Поиск → RAG авто |
| Кнопка «Обогатить» на карточке лида | **§9** Лиды + enrich |
| Авто-чанки без approve | **§4** |
