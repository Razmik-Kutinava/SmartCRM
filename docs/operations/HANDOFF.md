...
`Спринт:[Фаза 1→2] | Задача:[MAP выровнен] | Статус:таблицы единые, шапки, Поиск/RAG без дублей ✅ — ждём go продукт`
...
**DoD Ф1:** 4/6 ✅; п.5 ⚠️ микрофон; п.6 «ок в Ф2» — **ждём тебя**. Старт Ф2 по решению, не по автозакрытию.

**Порядок Ф2:** [`PRD_MAP.md` § Порядок Фазы 2](../product/PRD_MAP.md#порядок-фазы-2) — `go` только по номеру очереди.

**MAP-doc:** **закрыт** (2026-06-10) — легенда, DoD, сценарии, ⚠️ голос, хвост→BACKLOG, порядок Ф2, security/infra/agents/coverage, таблицы Лиды/Баллы/Голос/Поиск.

**Следующий продуктовый `go` (кандидаты):** очередь **5** §8 лидоген+голос **или** очередь **1** §1 Voice Layer **или** ручной микрофон E2E (Ф1).
