...
<p class="text-sm text-gray-400 mb-4 max-w-2xl">
	Сценарии intent Hermes3 (Ollama). По умолчанию без Groq — включите для сравнения с облаком.
</p>
...
<div class="mb-6 bg-gray-900/80 border border-gray-800 rounded-xl p-4 max-h-64 overflow-y-auto">
	<h3 class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">
		Что уйдёт в eval ({preview?.count ?? 0} фраз)
	</h3>
	{#if preview?.warning}
...
<div class="mb-4 flex flex-wrap items-center gap-4">
	...
	<span class="text-xs text-gray-600">Модели: {modelsForRun().join(', ')}</span>
</div>
...
<div class="mb-4 flex flex-wrap items-center gap-4">
	...
	>
		{evalRunning ? 'Тестируем…' : 'Запустить Hermes eval'}
	</button>
	{#if evalResults}
		...
		{#each Object.entries(evalResults.summary) as [model, s]}
			<div class="bg-gray-900 rounded-xl border border-gray-800 p-4">
				...
				<div class="text-xs text-gray-500 mt-1">
					{s.correct}/{s.total} верно · {s.avg_ms}ms
				</div>
			</div>
		{/each}
	</div>
			...
			<tbody class="divide-y divide-gray-800">
				{#each evalResults.results as row}
					<tr class="hover:bg-gray-800/50">
						<td class="px-4 py-3 text-gray-500 text-xs font-mono">{row.scenario_id ?? '—'}</td>
...
{:else if evalRunning}
	<div class="text-center py-8 text-gray-400 border border-gray-800 rounded-xl">
		Прогон {preview?.count ?? '…'} фраз через {modelsForRun().join(' + ')}…
	</div>
{:else}
	<div class="text-center text-gray-500 py-8">Выберите набор и нажмите «Запустить Hermes eval».</div>
{/if}
